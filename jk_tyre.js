...
const authToken = '8c1aad627a45767038388fdff622562a';
...
// Calculate the average of all parameter values
const paramValues = [parameter1, parameter2, parameter3, parameter4, parameter5, parameter6, parameter7, parameter8, parameter9, parameter10];
    
// Filter out non-numeric values and convert to numbers
const numericValues = paramValues.map(value => parseFloat(value)).filter(value => !isNaN(value));
...
if (numericValues.length > 0) {
  const average = numericValues.reduce((sum, value) => sum + value, 0) / numericValues.length;
    ...
    // Define the limits for the average (you can set these limits as needed)
    const lowerLimit = limitations[0].lower_limit; // Example lower limit
    const upperLimit = limitations[0].upper_limit; // Example upper limit

  

    // Check if the average exceeds the defined limits
    if (average < lowerLimit || average > upperLimit) {
      ...
      const emailBody = emailTemplate
        ...
        .replace('{{paramName}}', 'Average of Parameters' || '')
        .replace('{{paramValue}}', average || '');
